...
class D2YNoData extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Center(child: Padding(padding: const EdgeInsets.all(AppSpacing.xl), child: Column(mainAxisSize: MainAxisSize.min, children: [
      icon ?? Icon(Icons.inbox_outlined, size: 80, color: isDark ? AppColors.primary : AppColors.onPrimary),
      const SizedBox(height: AppSpacing.lg),
      Text(title ?? 'No Data', style: AppTextStyles.headlineSmall.copyWith(color: isDark ? AppColors.onPrimary : AppColors.primary), textAlign: TextAlign.center),
      if (message != null) ...[const SizedBox(height: AppSpacing.xs),
        Text(message!, style: AppTextStyles.bodyMedium.copyWith(color: isDark ? AppColors.secondary : AppColors.onSecondary), textAlign: TextAlign.center)],
